...
if(isset($_GET['start'])){
	...
	$Qtotal = mysql_query("select * from tb_shopcash_item");
	$total = mysql_num_rows($Qtotal);
	
	$Query = mysql_query("SELECT * FROM tb_shopcash_item ORDER BY id DESC LIMIT $start,$limit");
...
<?php
	...
	$sql="select * from tb_shopcash_item where id='".cl($id_edit)."' ";
...
?>
  ...
  <p class="text-right"><a href="edit-shopcash.php" class="btn btn-default" role="button"><span class="glyphicon glyphicon-plus"></span> เพิ่มไอเท็ม</a></p>
  ...
  <input type="submit" name="submit" class="btn btn-default" value="ตกลง" />
	  ...
	  if($_POST['submit'])
	  {
		  if($id_edit == "")
		  {
			  ...
			  {
				  $update_sql = mysql_query("INSERT INTO tb_shopcash_item(id_type,item_id,item_name,item_description,item_count,item_price) VALUES('".cl($_POST['type'])."','".cl($_POST['itemid'])."','".cl($_POST['name'])."','".cl($_POST['info'])."','".cl($_POST['itemcount'])."','".cl($_POST['itemprice'])."')");
				  if(!$update_sql)
				  {
		  ...
		  else
		  {
			  ...
			  {
				  $update_sql = mysql_query("UPDATE tb_shopcash_item SET
				  ...
				  else
				  {
					  ...
					  go("edit-shopcash.php?id_edit=".$id_edit."");
				  }
			  }
		  }
	  }
	  ...
	  if($id_del != "")
	  {
		  $delete_sql = mysql_query("DELETE FROM tb_shopcash_item WHERE id = '".cl($id_del)."'");
		  if(!$delete_sql)
		  {
			  popup("เกิดข้อผิดพลาดขณะรันข้อมูล");
		  }
...
echo"
 <tr >
    ...
    <td>$news_name</td>
	<td>$type_name</td>
    <td align='center'><a href=\"edit-shopcash.php?id_edit=$news_id\"><button class='btn btn-default'><span class='glyphicon glyphicon-edit'></span></button></a></td>
    <td align='center'><a href=\"edit-shopcash.php?id_del=$news_id\" onclick=\"return confirm('คุณต้องการลบจริงหรือไม่')\"><button class='btn btn-default'><span class='glyphicon glyphicon-remove'></span></button></a></td>
</tr>";
